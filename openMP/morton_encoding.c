...
void morton_encoding(unsigned long int *mcodes, unsigned int *codes, int N, int max_level){
    ...
    //make parallel: very easy section to parallelize
    int chunk = N/omp_get_num_procs();
    #pragma omp parallel for shared(mcodes, codes) private(i) num_threads(numThreads) schedule(dynamic, chunk)
    for(i=0; i<N; i++){
        mcodes[i] = mortonEncode_magicbits(codes[i*DIM], codes[i*DIM + 1], codes[i*DIM + 2]); // Compute the morton codes from the hash codes using the magic bits method
    }
}
